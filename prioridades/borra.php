...
if ($enlace) {
    ...
    if ($obtener){
        if (mysqli_num_rows($obtener)>0){
            $texto .= "<label for='idnombreprioridad'>Nombre Prioridad</label>";
            $fila = mysqli_fetch_array($obtener);
            $texto .= "<input type='text' class='form-control' id='idnombreprioridad' value='".$fila['nompri']."' readonly>";
            if ($fila['Activo']){
            ...
            $texto .= "</div><h2>NO SE HA ENCONTRADO LA PRIORIDAD</h2>";
        }
        echo $texto;
...
<?php
    if ($encontrado == 1){
        echo "<button type='button' id='borrarprioridad' class='btn btn-danger'><i class='fas fa-trash-alt'></i> Borrar</button>";
    }
